...
class MLPClassifier66:
    ...
    def normalize_meanstd(X):
        mean = [-0.3866577540106952, 0.3131016042780749, 0.9723262032085561, 11.140005347593583, -43.611909090909094, 33.24405347593583]
        std = [75.84739018176921, 74.9897990888454, 76.01213243253815, 2496.9076229290667, 2655.984538481507, 3484.031394269587] 
    ...
    def record_sample(self, sample):
        ...
        if totalAccAbs < 8000 and self.r_busy == False:
            ...
            if len(self.r_sample_buffer) >= self.window_size:
                ...
                # Write to csv
                self.imu_data_df.to_csv('w12_data/web_silin.csv', index=False)
            return

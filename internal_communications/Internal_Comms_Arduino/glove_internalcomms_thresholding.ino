...
unsigned long last_sent_handshake_time;
...
//        get_dmp_data(GLOVE_BEETLE_DATA); 
        GyroX = IMU.GyroX_moving_average_hull * GYRO_SCALING_FACTOR;
        GyroY = IMU.GyroY_moving_average_hull * GYRO_SCALING_FACTOR;
        GyroZ = IMU.GyroZ_moving_average_hull * GYRO_SCALING_FACTOR;
        
        if(abs(abs(GyroX) + abs(GyroY) + abs(GyroZ)) > 8000 and send_data == false){
//          Serial.println(abs(packet.GyroX + packet.GyroY + packet.GyroZ));
          send_data = true;
        }
        if(send_packet_count >= 60){
          send_packet_count = 0;
          send_data = false;
        }
        if(send_data == true){
...
//          send_to_internal_comms(GLOVE_BEETLE_DATA);
    ...
    case 'h':
      resetFlags();
      if (millis() - last_sent_handshake_time > 10) {
        sendHandshakeAck();
      }
      waitForHandshakeAck();
      break;
...
uint32_t calculateMpuCrc32(Mpupacket *pkt) {
  return custom_crc32(&pkt->type, 
    sizeof(pkt->type) +
    sizeof(pkt->padding_1) +
    sizeof(pkt->padding_2) +
...
void sendHandshakeAck() {
  ...
  last_sent_handshake_time = millis();
...
void sendMpuReady() {
  ...
  packet.padding_7 = 0;
  packet.padding_8 = 0;
  packet.crcsum = calculateMpuCrc32(&packet);
  Serial.write((uint8_t *)&packet, sizeof(packet));
}
//================ State Machine =================
